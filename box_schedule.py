...
def read_all():
    ...
    db = get_db()
    cursor = db.execute("""
        SELECT id, box_num, date, order_id
        FROM box_schedule
        ORDER BY date DESC
    """)
...
def create():
    ...
    required_fields = ['box_num', 'date', 'order_id']
    ...
    db = get_db()
    cursor = db.execute("""
        INSERT INTO box_schedule (box_num, date, order_id)
        VALUES (?, ?, ?)
    """, (
        data['box_num'],
        data['date'],
...
def update(schedule_id):
    """PUT /api/box_schedule/{schedule_id} - изменить запись"""
    data = request.get_json()
...
def delete(schedule_id):
    """DELETE /api/box_schedule/{schedule_id} - удалить запись"""
    db = get_db()
